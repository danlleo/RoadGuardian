...
namespace Core.AssetLoaderModule.Core.Scripts
{
    public class ResourceAssetLoaderService : IResourceAssetLoaderService
    {
        ...
        public bool HasLoadedAsset(string key, string groupName = "Default") =>
            _handlesContainerByGroupName.TryGetValue(groupName, out ResourcesGroupHandleContainer handleContainer) is
                true
            && handleContainer.CompletedHandles.ContainsKey(key);

        private async UniTask<TAsset> ProcessHandleAsync<TAsset>(string key,
            ResourcesGroupHandleContainer handleContainer) where TAsset : Object
        {
        ...
        private TAsset ProcessHandle<TAsset>(string key, ResourcesGroupHandleContainer handleContainer)
            where TAsset : Object
        {
            ResourceRequest handle = Resources.LoadAsync<TAsset>(key);
