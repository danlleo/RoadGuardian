...
namespace Core.AssetLoaderModule.Core.Scripts
{
    public class AddressableAssetLoaderService : IAddressablesAssetLoaderService
    {
        ...
        public async UniTask<TAsset> LoadAssetAsync<TAsset>(string key, string groupName = "Default")
            where TAsset : Object
        {
        ...
        {
            if (_handlesContainerByGroupName.TryGetValue(groupName,
                    out AddressablesGroupHandleContainer handleContainer) is false)
                return;

            foreach (KeyValuePair<string, List<AsyncOperationHandle>> allHandlesInContainer in handleContainer
                         .AllHandles)
        ...
        {
            foreach (KeyValuePair<string, AddressablesGroupHandleContainer> handlesContainer in
                     _handlesContainerByGroupName.ToList())
                ReleaseAssetsInGroup(handlesContainer.Key);
        }

        public bool HasLoadedAsset(string key, string groupName = "Default") =>
            _handlesContainerByGroupName.TryGetValue(groupName, out AddressablesGroupHandleContainer handleContainer) is
                true
            && handleContainer.CompletedHandles.ContainsKey(key);

        private async UniTask<TAsset> ProcessHandleAsync<TAsset>(string key,
            AddressablesGroupHandleContainer handleContainer) where TAsset : Object
        {
        ...
        private TAsset ProcessHandle<TAsset>(string key, AddressablesGroupHandleContainer handleContainer)
            where TAsset : Object
        {
            AsyncOperationHandle<TAsset> handle = Addressables.LoadAssetAsync<TAsset>(key);
        ...
        private void AddHandle<TAsset>(string key, AsyncOperationHandle<TAsset> handle,
            AddressablesGroupHandleContainer handleContainer) where TAsset : Object
        {
        ...
        {
            if (_handlesContainerByGroupName.TryGetValue(groupName,
                    out AddressablesGroupHandleContainer handleContainer))
